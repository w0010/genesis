...
var mouseX = 0, mouseY = 0, scroll = 0;
	...
	scene.background = new THREE.Color('hsl(212, 2%, 75%)');
	scene.fog = new THREE.Fog( 'hsl(0, 0%, 0%)' , 100, 1500 );
	var camera = new THREE.PerspectiveCamera(150, window.innerWidth / window.innerHeight, 10, 10000);
	...
	spotLight.position.set( 0, 500, 2000 ); //SpotLight.target defaults to 0, 0, 0
	scene.add(spotLight);
...
// geometry
	var geometry = new THREE.IcosahedronBufferGeometry( 1000, 0 );
...
function MouseMove(event) {
	...
	if (event.clientX < window.innerWidth / 2) {
		...
		mesh.rotation.x = event.clientX * 0.001;
	} else {
