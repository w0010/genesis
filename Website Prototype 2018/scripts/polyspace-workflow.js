...
var mouseX = 0, mouseY = 0;
	...
	scene.fog = new THREE.Fog( gunmetal, 1, 3500 );
...
// geometry
	var geometry = new THREE.TorusKnotBufferGeometry( 3500, 1000, 300, 100 );
...
function MouseMove(event) {
	...
	if (event.clientX < window.innerWidth / 2) {
		camera.position.x = ZOOM - event.clientX * ZOOMER;
		mesh.rotation.z = event.clientX * 0.001;
	} else {
		camera.position.x = ZOOM - window.innerWidth * ZOOMER + event.clientX * ZOOMER;
		mesh.rotation.z = event.clientX * 0.001;
	};
}; 
