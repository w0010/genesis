...
// INIT
	...
	camera = new THREE.PerspectiveCamera(150, window.innerWidth / window.innerHeight, 0.01, 10000);
	camera.position.z = ZOOM;
	renderer = new THREE.WebGLRenderer( { alpha: true, antialias: true } );
	renderer.setSize( window.innerWidth, window.innerHeight );
	renderer.setClearColor( 0x000000, 0 );
	var container = document.getElementById( 'core-container' );
	...
	spotLight.position.set( 0, 0, 10000 ); //SpotLight.target defaults to 0, 0, 0
	scene.add(spotLight);
...
//				//
	...
	var geometry = new THREE.BoxBufferGeometry( 500, 2000, 1000 );
...
function MouseMove(event) {
	...
	if (event.clientX < window.innerWidth / 2) {
		...
		mesh.rotation.y = event.clientX * 0.005;
	} else {
